...
function main() {
  try {
    ...
    // Remove duplicate symbols using Set
    var uniqueSymbols = [];
    var seenCodes = {};
    
    for (var i = 0; i < ptsData.length; i++) {
      var symbol = ptsData[i];
      if (!seenCodes[symbol.code]) {
        seenCodes[symbol.code] = true;
        uniqueSymbols.push(symbol);
      } else {
        Logger.log('Duplicate symbol detected and removed: ' + symbol.code);
      }
    }
    
    Logger.log('After duplicate removal: ' + uniqueSymbols.length + ' unique symbols');
    
    var allRows = [];
    
    // Process each unique symbol
    for (var i = 0; i < uniqueSymbols.length; i++) {
      var symbol = uniqueSymbols[i];
